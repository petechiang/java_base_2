...
public class Shili_input {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 1; i <= rows_dest; i++) {
			...
			//取出源表里的视力值并写入目标表对应的位置
			if(tcxxjh!=null) {
				rowi_dest.createCell(15).setCellValue(tcxxjh.get(11).toString());//m表示第几列
				rowi_dest.createCell(16).setCellValue(tcxxjh.get(12).toString());//m表示第几列
			}
